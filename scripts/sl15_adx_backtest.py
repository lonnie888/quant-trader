"""Compare: SL=-12% TP=+20% (current) vs SL=-15% TP=+20% vs SL=-15% + ADX filter."""
...
def compute_adx(df, period=14):
    """Compute ADX (Average Directional Index) for each bar."""
    high = df['high'].values
    low = df['low'].values
...
def backtest(sl_pct, tp_pct, daily_loss_limit, adx_filter=0.0, hold_bars=24):
    ...
    for sym, df in symbols:
        ...
        if not pump_idx:
            continue
        # Precompute ADX once
        adx = compute_adx(df) if adx_filter > 0 else None
        in_pos = False
        for i in range(n):
            if not in_pos:
                if s[i] == 1:
                    if not any(0 <= i - pi <= 1 for pi in pump_idx):
                        continue
                    # ADX filter: skip if ADX below threshold (choppy market)
                    if adx is not None and i < len(adx) and adx[i] < adx_filter:
                        continue
                    in_pos = True
...
VARIANTS = {
    # baseline
    "v0.4.0 SL=-12% TP=+20%": (-0.12, 0.20, 0.50, 0.0),
    # only SL change
    "SL=-15% TP=+20%":          (-0.15, 0.20, 0.50, 0.0),
    "SL=-18% TP=+20%":          (-0.18, 0.20, 0.50, 0.0),
    "SL=-20% TP=+20%":          (-0.20, 0.20, 0.50, 0.0),
    # ADX filter (only trade trending markets)
    "SL=-12% ADX>15":           (-0.12, 0.20, 0.50, 15.0),
    "SL=-12% ADX>20":           (-0.12, 0.20, 0.50, 20.0),
    "SL=-12% ADX>25":           (-0.12, 0.20, 0.50, 25.0),
    "SL=-15% ADX>20":           (-0.15, 0.20, 0.50, 20.0),
    "SL=-15% ADX>25":           (-0.15, 0.20, 0.50, 25.0),
    "SL=-18% ADX>20":           (-0.18, 0.20, 0.50, 20.0),
}

for name, (sl, tp, dl, adx_f) in VARIANTS.items():
    raw, passed, blocked = backtest(sl, tp, dl, adx_f)
