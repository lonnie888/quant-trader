"""Compare cooldown variants: 12/24/48/96 (current=12) with bars_since=1."""
...
def backtest(cooldown):
    p = dict(BASE)
    p["cooldown"] = cooldown
    strategy = PumpPullbackStrategy(p)
    raw = []
        ...
        for i in range(n):
            if not in_pos:
                if s[i] == 1:
                    ...
                    entry_idx = i
                    held = 0
                    sl_p = entry_p * (1 - float(p["stop_loss_pct"]))
                    tp_p = entry_p * (1 + float(p["take_profit_pct"]))
            else:
                held += 1
                if low[i] <= sl_p:
                    ...
                    reason = "tp"
                    in_pos = False
                elif held >= int(p["hold_bars"]) or s[i] == 0:
                    exit_p = close[i] * (1 - SLIPPAGE)
                    reason = "time" if held >= int(p["hold_bars"]) else "signal"
                    in_pos = False
                else:
                    continue
...
for name, cd in VARIANTS.items():
    raw, passed, blocked = backtest(cd)
