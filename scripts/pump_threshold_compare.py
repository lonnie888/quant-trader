"""Compare pump_threshold variants: 0.13 (current) vs 0.20 vs 0.25."""
...
print(f"{'变体':<28} {'原始':>6} {'通过':>6} {'阻挡率':>7} {'总收益%':>10} {'胜率%':>7} {'盈亏比':>7} {'PF':>6} {'Sharpe':>7}")
print("-" * 100)
for name, pth in VARIANTS.items():
    p = dict(BASE)
    p["pump_threshold"] = pth
    raw, passed, blocked = backtest(p)
    ...
    print(f"{name:<28} {len(raw):>6} {len(passed):>6} {blocked/(blocked+len(passed))*100:>6.1f}% {total_ret:>9.1f}% {win_rate:>6.1f}% {pl_ratio:>6.2f} {pf:>5.2f} {sharpe:>6.3f}")
